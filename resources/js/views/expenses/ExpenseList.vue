...
const fetchExpenses = async (page = 1) => {
  ...
  try {
    ...
    uiStore.showToast('Failed to fetch expenses', 'error');
  } finally {
    loading.value = false;
  }
};
...
const openPaymentModal = (expense) => {
  ...
  paymentForm.value = {
    amount: expense.amount - expense.paid_amount,
    payment_method: 'cash',
...
const submitPayment = async () => {
  if (!selectedExpense.value || paymentForm.value.amount <= 0) return;

  payingExpense.value = true;
  try {
    await api.post(`/expenses/${selectedExpense.value.id}/payment`, paymentForm.value);
    uiStore.showToast('Payment recorded successfully', 'success');
    showPaymentModal.value = false;
    fetchExpenses(meta.value.current_page);
    fetchStats();
  } catch (error) {
    uiStore.showToast('Failed to record payment', 'error');
  } finally {
    payingExpense.value = false;
  }
};
...
watch([search, categoryFilter, statusFilter, dateFrom, dateTo], () => {
  fetchExpenses(1);
});
  ...
  <div>
    <div class="flex items-center justify-between mb-6">
      <div>
        ...
        <p class="text-gray-500">Track temple expenses (utilities, salaries, maintenance, etc.)</p>
      </div>
      <div class="flex gap-3">
        <Button variant="outline" @click="router.push('/expenses/categories')">
          Manage Categories
        </Button>
        <Button variant="primary" @click="router.push('/expenses/new')">
    ...
    <Card>
      <Table :columns="columns" :data="expenses" :loading="loading">
        ...
        <template #paid_to="{ row }">
          {{ row.paid_to || '-' }}
        </template>
        <template #amount="{ row }">
          <div>
            <div class="font-medium">{{ formatAmount(row.amount) }}</div>
            <div v-if="row.payment_status !== 'paid'" class="text-xs text-red-600">
              Due: {{ formatAmount(row.amount - row.paid_amount) }}
            </div>
          </div>
        </template>
    ...
    <Modal :show="showPaymentModal" title="Record Payment" @close="showPaymentModal = false">
      <div v-if="selectedExpense" class="space-y-4">
        <div class="p-3 bg-gray-50 rounded-lg">
          <p class="text-sm text-gray-600">Expense: {{ selectedExpense.expense_number }}</p>
          <p class="text-sm text-gray-600">Description: {{ selectedExpense.description }}</p>
          <p class="font-medium">Due: {{ formatAmount(selectedExpense.amount - selectedExpense.paid_amount) }}</p>
        </div>
        ...
        <div>
          ...
          <input
            ...
            type="number"
            min="0.01"
            :max="selectedExpense.amount - selectedExpense.paid_amount"
            step="0.01"
        ...
        <div class="flex justify-end gap-3 pt-4">
          ...
          <Button variant="primary" :loading="payingExpense" @click="submitPayment">Record Payment</Button>
        </div>
      </div>
    </Modal>
  </div>
